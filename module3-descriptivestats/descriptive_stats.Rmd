...
The `fivenum()` function returns Tukey's five number summary: minimum value, lower-hinge (25th percentile), upper-hinge (75th percentile), and maximum value for the input data. Let's use the `fivenum()` function for the age variable in the gss dataset. 

```{r fivenum, exercise = TRUE}
fivenum(gss$age)
```
Please note, that the `fivenum()` function only accepts numeric input values. You will recieve an error if you provide data that are factors or characters.
...
We can find the mean by providing a vector of numeric values to the built in R function `mean()`. Note, this function does not exlude `NA` cases so we have to tell R to only include complete observations.

```{r mean, exercise = TRUE}
mean(gss$age, na.rm = TRUE)
```

Using mean as a measure of central tendency is appropriate if we have interval/ratio variables that normally distributed with no outliers. If the data is skewed, median is a more appropriate measure of central tendency to calculate for interval/ratio variables.

### `median()`

Like the `mean()` function, we can find the median by providing a vector of numeric values to the built in R function `median()`. Note, this function does not exlude `NA` cases so we have to tell R to only include complete observations.
...
Unlike `mean()` there is no built in R function to calculate the mode. However, you can install and load the package `DescTools` package and use their `Mode()` function to calculate the mode. Alternatively, the descriptive statistics functions we have discusssed provides the modes for numeric variables. Mode is the most appropriate measure of central tendency when we have nominal (count) data.
...
The `var()` function calculates the variance of a numeric vector. Variance measures how much data are spread out and is mathematically defined as the average of squared differences from the mean. Note, this function does not exlude `NA` cases so we have to tell R to only include complete observations.

```{r var, exercise = TRUE}
var(gss$age, na.rm = TRUE)
```

Overall, variance is difficult to interpret to describe dispersion. Instead we take the square root of the variance to calculate the standard deviation (which is the most commonly used measure of dispersion). 

### `sd()`

The `sd()` function calculates the standard deviation of a numeric vector. Standard deviation also measures how much data are spread out and is mathematically defined as each data point's deviation relative to the mean. Note, this function does not exlude `NA` cases so we have to tell R to only include complete observations.
...
The range is also another meausure of disperstion and be calculated using `range()` function. The output of this functions tells us the minimum value of a numeric vector as well as the maximum value. 
...
The frequency table for the tvhours variable isn't too clutered like the age variable was in the initial frequency table we created. The output is useful as is for tvhours.
