...
In this course we learned how to **import**, **wrangle**, and **visualize** our data. In this final module, we will learn how to generate basic descriptives statistics.
...
The difference between a population and sample is one of the first concepts taught in statistics and research methods courses. A **population** is a whole and complete group you want to make conclusions about and a **sample** is a subset of the population you're interested in. Researchers often collect data from a sample to make conclusions (or draw inferences) about the entire population of interest.
...
For example, all undergraduate students at the University of Tennessee, Knoxville would be considered the entire undergraduate population of UTK. However, a selection of 200 undergraduate students from UTK would be considered a sample of the population.
...
Researchers want to examine relationship between variables as guided by their specific research question. This examination is done by specifying an independent variable and a dependent variable. An **independent variable** is a variable that varies or can be manipulated to explore the effect/relationship it has with a **dependent variable**. As the name implies, independent variables are variables are independent of other variables/characteristics. Meaning that no other variables in the research study impact the independent variable. 

There are two primary types of independent variables: 1) experimental independent variables and 2) subject variables. An experiment independent variable is controlled and can be directly manipulated by a researcher (treatment conditions, for example). A subject variable is not controlled nor can be manipulated by a researcher but is a natural occuring variable within a subject/participant (e.g., height, weight, education, age).

### Dependent Variable

A **dependent variable** is also called the **outcome variable** and is hypothesized to change as a result of changing/manipulating the **independent variable**. In other words, the dependent variable is dependent on the independent variable. A dependent variable is the outcome of interest in which a researcher wishes to examine how the dependent variable changes as the independent is manipulated or varied. 

### Example

A classic example of independent and dependent variables is examining the relationship between students' study hours and their test score. If a researcher wishes to examine the relationship between a study hours and test scores, the main independent variable would be study hours and the dependent (or outcome) variable would be test scores. 

## Levels of Measurement

In statistics, variables can be measured on one of four levels of measurement:
...
These types of measurement are listed in increasing order of detail information they provide starting with nominal variables.
...
- Simple random sampling (SRS): Each member of a population has an equal and fair probability of being selected. This is one of the most convenience and simple random selection techniques.
...
### El Fin
Good job on completing the second of two tutorials in the final module of this course! Please move onto the next tutorial within this module on Canvas where we will learn how generate descriptive statistics in R.
