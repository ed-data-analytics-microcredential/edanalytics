...
# Generate descriptive statistics for each variable in the hsbtransformed dataset.
describe(hsbtransformed)
summary(hsbtransformed)

# Please list the level of measurement for each variable (e.g., nominal, ordinal, interval, ratio) and your justification.
# id is nominal because it is a unique identifier for each student
# gender, ses, race, program, and schooltype are nominal variables because they are all categorical (or factor) variables
# ses is ordinal because it is a categorical variable with an order (low < middle < high)
# read, write, math, science, socialst and final score are interval variables becuase they are continous but do not have a true zero point (i.e., does a 0 for reading score mean someone has no reading ability?)

# Please list and report the appropriate central tendency measure for each variable and your justification.
# id - none
# gender, ses, race, program, and schooltype - mode because they are categorical variables
# read, write, math, science, socialst and final score - mean or median because they are interval variables (depending on the level of skewness, check histograms to be sure)
hist(hsbtransformed$read) 
hist(hsbtransformed$write) 
hist(hsbtransformed$math)
hist(hsbtransformed$science)
hist(hsbtransformed$socialst)
hist(hsbtransformed$finalscore)


# Report the skewness and kurtosis for each variable.
describe(hsbtransformed)

# Please pick three variables and generate a frequency table for each of these
# variables. Is there anything interesting based on these frequency tables?
table(hsbtransformed$gender)
table(hsbtransformed$program)
table(hsbtransformed$math)
# Notice how tables are less useful with interval/ratio data

# What do you find interesting or curious about the statistics generated from
# this dataset?
# Answers will vary. 