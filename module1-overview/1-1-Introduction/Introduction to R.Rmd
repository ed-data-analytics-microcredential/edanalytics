...
**Variables.** 
Now that you know how R classifies some of the basic information types, we will almost always want to 'store' this information as a variable or as an object. In programming, variables allow us to store information and associate that information with a name. In R, we assign objectvariables by using the assignment operator, an arrow sign (<-) made with a less-than signcarat and a dash. 

Below is an example demonstrating how to assign data types to a variable or object. Run the first line of code written below. You can see that R gives no output. That's because you simply stored/assigned 9 to the word apple. Now go ahead and type in apple under the line of code written. Take some time to think about the output R will give, if any at all. Go ahead and run these two lines of code to show the output.
```{r  assignmentoperator, exercise = TRUE}
apple <- 9  #this assigns or 'stores' 9 (a numeric data type) to the word 'apple' (a character data type)


```
<br>
**Vectors.**
We mentioned Vectors when we introduced data types earlier. In R, vectors are a list-like structure that contain items of the same data type. In the example below, we assign a list of character data types (names of months in this case) to a vector called `spring_months`.

Like the example above, if you run the line of code written below, R gives no output due to the same reason as above. Take some time to think about the output R will give, if any at all, before typing in `spring_months` under the line of code written and pressing "Run Code."
